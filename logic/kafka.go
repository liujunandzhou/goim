...
func mpushKafka(serverId int32, keys []string, msg []byte) (err error) {
	...
	producer.Input() <- &sarama.ProducerMessage{Topic: Conf.KafkaPushsTopic, Value: sarama.ByteEncoder(vBytes)}
	return
}
...
func broadcastRoomKafka(rid int32, msg []byte, ensure bool) (err error) {
	...
	producer.Input() <- &sarama.ProducerMessage{Topic: Conf.KafkaPushsTopic, Key: sarama.ByteEncoder(ridBytes[:]), Value: sarama.ByteEncoder(vBytes)}
	return
}
